...
part of 'created_thread_response.dart';

// **************************************************************************
// CopyWithGenerator
// **************************************************************************

abstract class _$CreatedThreadResponseCWProxy {
  CreatedThreadResponse id(String id);

  CreatedThreadResponse type(ChannelTypes type);

  CreatedThreadResponse lastMessageId(
    GetEntitlementsSkuIdsParameterOneOfInner? lastMessageId,
  );

  CreatedThreadResponse flags(int flags);

  CreatedThreadResponse lastPinTimestamp(DateTime? lastPinTimestamp);

  CreatedThreadResponse guildId(String guildId);

  CreatedThreadResponse name(String name);

  CreatedThreadResponse parentId(
    GetEntitlementsSkuIdsParameterOneOfInner? parentId,
  );

  CreatedThreadResponse rateLimitPerUser(int? rateLimitPerUser);

  CreatedThreadResponse bitrate(int? bitrate);

  CreatedThreadResponse userLimit(int? userLimit);

  CreatedThreadResponse rtcRegion(String? rtcRegion);

  CreatedThreadResponse videoQualityMode(VideoQualityModes? videoQualityMode);

  CreatedThreadResponse permissions(String? permissions);

  CreatedThreadResponse ownerId(String ownerId);

  CreatedThreadResponse threadMetadata(ThreadMetadataResponse threadMetadata);

  CreatedThreadResponse messageCount(int messageCount);

  CreatedThreadResponse memberCount(int memberCount);

  CreatedThreadResponse totalMessageSent(int totalMessageSent);

  CreatedThreadResponse appliedTags(List<String>? appliedTags);

  CreatedThreadResponse member(ThreadMemberResponse? member);

  /// This function **does support** nullification of nullable fields. All `null` values passed to `non-nullable` fields will be ignored. You can also use `CreatedThreadResponse(...).copyWith.fieldName(...)` to override fields one at a time with nullification support.
  ///
  /// Usage
  /// ```dart
  /// CreatedThreadResponse(...).copyWith(id: 12, name: "My name")
  /// ````
  CreatedThreadResponse call({
    String id,
    ChannelTypes type,
    GetEntitlementsSkuIdsParameterOneOfInner? lastMessageId,
    int flags,
...
/// Proxy class for `copyWith` functionality. This is a callable class and can be used as follows: `instanceOfCreatedThreadResponse.copyWith(...)`. Additionally contains functions for specific fields e.g. `instanceOfCreatedThreadResponse.copyWith.fieldName(...)`
class _$CreatedThreadResponseCWProxyImpl
    implements _$CreatedThreadResponseCWProxy {
  const _$CreatedThreadResponseCWProxyImpl(this._value);

  final CreatedThreadResponse _value;

  @override
  CreatedThreadResponse id(String id) => this(id: id);

  @override
  CreatedThreadResponse type(ChannelTypes type) => this(type: type);
  ...
  @override
  CreatedThreadResponse flags(int flags) => this(flags: flags);
  ...
  @override
  CreatedThreadResponse guildId(String guildId) => this(guildId: guildId);

  @override
  CreatedThreadResponse name(String name) => this(name: name);
  ...
  @override
  CreatedThreadResponse bitrate(int? bitrate) => this(bitrate: bitrate);

  @override
  CreatedThreadResponse userLimit(int? userLimit) => this(userLimit: userLimit);
  ...
  @override
  CreatedThreadResponse ownerId(String ownerId) => this(ownerId: ownerId);
...
extension $CreatedThreadResponseCopyWith on CreatedThreadResponse {
  /// Returns a callable class that can be used as follows: `instanceOfCreatedThreadResponse.copyWith(...)` or like so:`instanceOfCreatedThreadResponse.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$CreatedThreadResponseCWProxy get copyWith =>
      _$CreatedThreadResponseCWProxyImpl(this);
}
...
CreatedThreadResponse _$CreatedThreadResponseFromJson(
  ...
  ($checkedConvert) {
    ...
    final val = CreatedThreadResponse(
...
Map<String, dynamic> _$CreatedThreadResponseToJson(
  CreatedThreadResponse instance,
) => <String, dynamic>{
  'id': instance.id,
