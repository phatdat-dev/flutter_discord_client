...
abstract class _$MessageResponseCWProxy {
  MessageResponse type(MessageType type);

  MessageResponse content(String content);

  MessageResponse mentions(List<UserResponse> mentions);

  MessageResponse mentionRoles(Set<String> mentionRoles);

  MessageResponse attachments(List<MessageAttachmentResponse> attachments);

  MessageResponse embeds(List<MessageEmbedResponse> embeds);

  MessageResponse timestamp(DateTime timestamp);

  MessageResponse editedTimestamp(DateTime? editedTimestamp);

  MessageResponse flags(int flags);

  MessageResponse components(
    List<BasicMessageResponseComponentsInner> components,
  );

  MessageResponse stickers(List<GetSticker200Response>? stickers);

  MessageResponse stickerItems(List<MessageStickerItemResponse>? stickerItems);

  MessageResponse id(String id);

  MessageResponse channelId(String channelId);

  MessageResponse author(UserResponse author);

  MessageResponse pinned(bool pinned);

  MessageResponse mentionEveryone(bool mentionEveryone);

  MessageResponse tts(bool tts);

  MessageResponse call(MessageCallResponse? call);

  MessageResponse activity(Object? activity);

  MessageResponse application(BasicApplicationResponse? application);

  MessageResponse applicationId(String? applicationId);

  MessageResponse interaction(MessageInteractionResponse? interaction);

  MessageResponse nonce(BasicMessageResponseNonce? nonce);

  MessageResponse webhookId(String? webhookId);

  MessageResponse messageReference(MessageReferenceResponse? messageReference);

  MessageResponse thread(ThreadResponse? thread);

  MessageResponse mentionChannels(
    List<BasicMessageResponseMentionChannelsInner>? mentionChannels,
  );

  MessageResponse roleSubscriptionData(
    MessageRoleSubscriptionDataResponse? roleSubscriptionData,
  );

  MessageResponse purchaseNotification(
    PurchaseNotificationResponse? purchaseNotification,
  );

  MessageResponse position(int? position);

  MessageResponse resolved(ResolvedObjectsResponse? resolved);

  MessageResponse poll(PollResponse? poll);

  MessageResponse sharedClientTheme(
    BasicMessageResponseSharedClientTheme? sharedClientTheme,
  );

  MessageResponse interactionMetadata(
    BasicMessageResponseInteractionMetadata? interactionMetadata,
  );

  MessageResponse messageSnapshots(
    List<MessageSnapshotResponse>? messageSnapshots,
  );

  MessageResponse reactions(List<MessageReactionResponse>? reactions);

  MessageResponse referencedMessage(
    MessageResponseReferencedMessage? referencedMessage,
  );

  /// This function **does support** nullification of nullable fields. All `null` values passed to `non-nullable` fields will be ignored. You can also use `MessageResponse(...).copyWith.fieldName(...)` to override fields one at a time with nullification support.
  ///
  /// Usage
  /// ```dart
...
/// Proxy class for `copyWith` functionality. This is a callable class and can be used as follows: `instanceOfMessageResponse.copyWith(...)`. Additionally contains functions for specific fields e.g. `instanceOfMessageResponse.copyWith.fieldName(...)`
class _$MessageResponseCWProxyImpl implements _$MessageResponseCWProxy {
  ...
  @override
  MessageResponse type(MessageType type) => this(type: type);

  @override
  MessageResponse content(String content) => this(content: content);
  ...
  @override
  MessageResponse timestamp(DateTime timestamp) => this(timestamp: timestamp);
  ...
  @override
  MessageResponse flags(int flags) => this(flags: flags);
  ...
  @override
  MessageResponse stickerItems(
    List<MessageStickerItemResponse>? stickerItems,
  ) => this(stickerItems: stickerItems);

  @override
  MessageResponse id(String id) => this(id: id);

  @override
  MessageResponse channelId(String channelId) => this(channelId: channelId);

  @override
  MessageResponse author(UserResponse author) => this(author: author);

  @override
  MessageResponse pinned(bool pinned) => this(pinned: pinned);
  ...
  @override
  MessageResponse tts(bool tts) => this(tts: tts);

  @override
  MessageResponse call(MessageCallResponse? call) => this(call: call);

  @override
  MessageResponse activity(Object? activity) => this(activity: activity);
  ...
  @override
  MessageResponse nonce(BasicMessageResponseNonce? nonce) => this(nonce: nonce);

  @override
  MessageResponse webhookId(String? webhookId) => this(webhookId: webhookId);

  @override
  MessageResponse messageReference(
    MessageReferenceResponse? messageReference,
  ) => this(messageReference: messageReference);

  @override
  MessageResponse thread(ThreadResponse? thread) => this(thread: thread);
  ...
  @override
  MessageResponse position(int? position) => this(position: position);
  ...
  @override
  MessageResponse poll(PollResponse? poll) => this(poll: poll);
  ...
  MessageResponse call({
    ...
    Object? referencedMessage = const $CopyWithPlaceholder(),
  }) {
    return MessageResponse(
      ...
      referencedMessage: referencedMessage == const $CopyWithPlaceholder()
          ? _value.referencedMessage
          // ignore: cast_nullable_to_non_nullable
          : referencedMessage as MessageResponseReferencedMessage?,
    );
  }
}

extension $MessageResponseCopyWith on MessageResponse {
  /// Returns a callable class that can be used as follows: `instanceOfMessageResponse.copyWith(...)` or like so:`instanceOfMessageResponse.copyWith.fieldName(...)`.
  ...
  ($checkedConvert) {
    ...
    final val = MessageResponse(
      ...
      referencedMessage: $checkedConvert(
        ...
        (v) => v == null
            ? null
            : MessageResponseReferencedMessage.fromJson(
                v as Map<String, dynamic>,
              ),
      ),
    );
...
Map<String, dynamic> _$MessageResponseToJson(MessageResponse instance) =>
    <String, dynamic>{
      ...
      'referenced_message': ?instance.referencedMessage?.toJson(),
    };
